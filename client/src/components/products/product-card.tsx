...
export function ProductCard({ product, onCompare }: ProductCardProps) {
  return (
    <Card className="group overflow-hidden border-muted hover:border-primary/50 transition-all duration-300 hover:shadow-lg hover:shadow-primary/5 bg-card h-full flex flex-col relative">
      {/* Badges */}
      <div className="absolute top-3 right-3 z-10 flex flex-col gap-2">
        {product.isNew && <Badge className="bg-blue-500 hover:bg-blue-600">جديد</Badge>}
        {product.isBestSeller && <Badge className="bg-amber-500 hover:bg-amber-600">الأكثر مبيعاً</Badge>}
        {product.ecoFriendly && (
      ...
      <div className="relative pt-[100%] bg-muted/20 overflow-hidden">
        ...
        <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-background/90 to-transparent translate-y-full group-hover:translate-y-0 transition-transform duration-300 flex justify-center gap-2">
          <Button size="sm" variant="secondary" className="rounded-full" onClick={() => onCompare?.(product)}>
            مقارنة
          </Button>
          <Button size="icon" variant="secondary" className="rounded-full text-red-500 hover:text-red-600 hover:bg-red-50">
