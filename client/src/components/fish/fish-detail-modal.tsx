...
export function FishDetailModal({ fish, open, onOpenChange }: FishDetailModalProps) {
  // Always render Dialog to allow Radix to handle open/close properly
  // Guard against null fish inside the content
  if (!fish) {
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-lg p-6">
          <DialogHeader>
            <DialogTitle>جارٍ التحميل...</DialogTitle>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] p-0 relative gap-0 z-[1000]" aria-describedby="fish-detail-desc">
