...
import { ArrowRight, Star, Truck, ShieldCheck, Phone, Leaf, Droplets, Thermometer, Package, ShoppingCart } from "lucide-react";
...
function ProductCard({ product }: { product: any }) {
  return (
    <Card className="group border-none shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 overflow-hidden bg-card h-full flex flex-col">
      <div className="relative aspect-[4/3] overflow-hidden bg-muted">
        {product.originalPrice && (
          <span className="absolute top-3 right-3 z-10 bg-rose-500 text-white text-xs font-bold px-2 py-1 rounded-md shadow-lg">
            خصم
          </span>
        )}
        <div className="absolute top-3 left-3 z-10 flex gap-1">
          {product.difficulty && <DifficultyBadge level={product.difficulty} className="text-[10px] px-2 py-1 h-6 backdrop-blur-md bg-white/80 dark:bg-black/50" />}
        </div>
        <img 
          src={product.image} 
          alt={product.name} 
        ...
        />
        <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-4 group-hover:translate-y-0 flex justify-center">
          <Button size="sm" className="w-full bg-white text-slate-900 hover:bg-white/90 font-bold shadow-lg">
            نظرة سريعة
          </Button>
        </div>
      </div>
      <CardContent className="p-5 flex-1 flex flex-col">
        <div className="text-xs text-muted-foreground mb-2 font-medium uppercase tracking-wider">{product.brand}</div>
        <h3 className="font-bold text-foreground line-clamp-2 mb-3 text-lg leading-snug group-hover:text-primary transition-colors">
          {product.name}
        </h3>
        <div className="flex items-center gap-2 mb-4 mt-auto">
          <div className="flex text-amber-400">
            {[...Array(5)].map((_, i) => (
              <Star key={i} className={`h-3.5 w-3.5 ${i < Math.floor(product.rating) ? "fill-current" : "text-slate-200 dark:text-slate-700"}`} />
            ))}
          </div>
        ...
        <div className="flex items-baseline gap-2 pt-2 border-t border-border/50">
          <span className="text-xl font-bold text-primary">{product.price.toLocaleString()} د.ع.</span>
          {product.originalPrice && (
            <span className="text-sm text-muted-foreground line-through decoration-rose-500/50">{product.originalPrice.toLocaleString()}</span>
          )}
        </div>
      </CardContent>
      <CardFooter className="p-5 pt-0">
        <Button className="w-full bg-foreground hover:bg-primary text-background hover:text-foreground transition-colors h-12 font-bold">
