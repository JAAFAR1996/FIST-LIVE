import { neon } from "@neondatabase/serverless";

async function applyNewsletterMigration() {
  const databaseUrl = process.env.DATABASE_URL;

  if (!databaseUrl) {
    console.error("❌ DATABASE_URL غير موجود في متغيرات البيئة");
    process.exit(1);
  }

  const sql = neon(databaseUrl);

  try {
    console.log("\n⏳ تطبيق Migration لـ Newsletter Subscriptions\n");
    console.log("═".repeat(50));

    // Check if table already exists
    console.log("\n1️⃣ التحقق من وجود الجدول...");
    const tableCheck = await sql`
      SELECT EXISTS (
        SELECT FROM information_schema.tables
        WHERE table_schema = 'public'
        AND table_name = 'newsletter_subscriptions'
      ) as exists;
    `;

    if (tableCheck[0]?.exists) {
      console.log("✅ جدول newsletter_subscriptions موجود بالفعل");
      const count = await sql`SELECT COUNT(*) as count FROM newsletter_subscriptions`;
      console.log(`📊 عدد السجلات الحالية: ${count[0]?.count || 0}`);
      console.log("\n✅ Migration مطبّق مسبقاً - لا حاجة لإعادة التطبيق\n");
      return;
    }

    console.log("⚠️  الجدول غير موجود - سيتم إنشاؤه الآن...\n");

    // Create table
    console.log("2️⃣ إنشاء جدول newsletter_subscriptions...");
    await sql`
      CREATE TABLE newsletter_subscriptions (
        id TEXT PRIMARY KEY DEFAULT gen_random_uuid(),
        email TEXT NOT NULL UNIQUE,
        is_active BOOLEAN DEFAULT true,
        created_at TIMESTAMP NOT NULL DEFAULT NOW(),
        updated_at TIMESTAMP NOT NULL DEFAULT NOW()
      )
    `;
    console.log("✅ تم إنشاء الجدول");

    // Create indexes
    console.log("\n3️⃣ إنشاء الفهارس...");
    await sql`CREATE INDEX idx_newsletter_email ON newsletter_subscriptions(email)`;
    await sql`CREATE INDEX idx_newsletter_active ON newsletter_subscriptions(is_active)`;
    console.log("✅ تم إنشاء الفهارس");

    // Verify
    console.log("\n4️⃣ التحقق من النتيجة...");
    const finalCount = await sql`SELECT COUNT(*) as count FROM newsletter_subscriptions`;
    console.log(`📊 عدد السجلات: ${finalCount[0]?.count || 0}`);

    console.log("\n" + "═".repeat(50));
    console.log("✅ تم تطبيق Migration بنجاح!");
    console.log("═".repeat(50) + "\n");

  } catch (error: any) {
    console.error("\n❌ خطأ في تطبيق Migration:");
    console.error(error.message);
    console.error("\nالتفاصيل:", error);
    process.exit(1);
  }
}

applyNewsletterMigration();
